...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # [创新点 1]：双重约束剪枝参数定义
    # ================================================================
    count_start_iter = opt.iterations - 3000  # 最后 3000 轮统计
    prune_vis_threshold = 100  # 可见性计数阈值
    prune_scale_threshold = 0.002  # 尺度阈值 (参考 Reduced-3DGS)
    # ================================================================
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # [创新点 1]：累加可见性统计
        if iteration > count_start_iter:
            # 如果点云扩增了，自动扩展统计张量的长度
            if gaussians.visibility_stats.shape[0] != gaussians.get_xyz.shape[0]:
                new_stats = torch.zeros(gaussians.get_xyz.shape[0], device="cuda")
                # 保留旧的统计数据
                old_size = min(gaussians.visibility_stats.shape[0], new_stats.shape[0])
                new_stats[:old_size] = gaussians.visibility_stats[:old_size]
                gaussians.visibility_stats = new_stats
